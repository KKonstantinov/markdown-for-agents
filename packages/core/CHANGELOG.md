# markdown-for-agents

## 1.4.0

### Minor Changes

- ef84782: Strip `hidden` and `aria-hidden="true"` elements in extraction mode, with a `keepHidden` option to opt out. Content a page hides from rendering or from the accessibility tree is decorative, collapsed, or a layout duplicate of visible text, and it used to leak into the
  Markdown as run-together noise.

    Collapse `<br>` and wrapped source lines inside headings to a single space. A Markdown heading is one line, so the trailing double-space break that `<br>` produces elsewhere used to split a heading into a heading and a stray paragraph.

    The Python package gains the same behaviour and a matching `keep_hidden` option.

## 1.3.4

### Patch Changes

- 9e1b395: fix arbitrary-value brackets

## 1.3.3

### Patch Changes

- 5f608cd: docs update

## 1.3.2

### Patch Changes

- 9705464: docs update

## 1.3.1

### Patch Changes

- 58f5fd0: add custom timing header

## 1.3.0

### Minor Changes

- 6fcacb3: add serverTiming performance benchmark

## 1.2.0

### Minor Changes

- 645ba5f: add node 22 support

## 1.1.4

### Patch Changes

- cd07bf6: docs update

## 1.1.3

### Patch Changes

- 066c10c: docs update

## 1.1.2

### Patch Changes

- 0f4b588: docs update

## 1.1.1

### Patch Changes

- 2d0aba2: docs update

## 1.1.0

### Minor Changes

- c27f3ae: npm provenance added

## 1.0.1

### Patch Changes

- version updates

## 1.0.0

### Major Changes

- major version bump

## 0.3.3

### Patch Changes

- pnpm audit fix

## 0.3.2

### Patch Changes

- remove custom typing for middleware packages

## 0.3.1

### Patch Changes

- jsr include fix

## 0.3.0

### Minor Changes

- jsr publishing bump

## 0.2.1

### Patch Changes

- frontmatter read title - nextjs streaming fix

## 0.2.0

### Minor Changes

- 367a912: add front matter, content signals features

## 0.1.3

### Patch Changes

- add etag, vary accept header support

## 0.1.2

### Patch Changes

- readme update

## 0.1.1

### Patch Changes

- initial publish
