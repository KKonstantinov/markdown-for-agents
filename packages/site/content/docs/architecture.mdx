...
Each middleware has its own framework dependency (Express, Fastify, Hono, Next.js). Separate packages avoid pulling in unused framework
types and keep install sizes minimal. All middleware packages depend on `markdown-for-agents` (the core library) as a regular dependency.
