...
description: Strip navigation, ads, sidebars, and cookie banners automatically to get just the main content.
---

# Content Extraction

When converting full web pages, you typically want the article content without navigation, ads, sidebars, cookie banners, and other
boilerplate. The extraction module prunes the DOM tree before conversion, keeping only meaningful content.

## Basic Usage

> **Tip:** See extraction in action in the [playground](/playground) - toggle "Extract content" to compare output with and without
> extraction.
...
| -------------------- | ----------------------------------------------------------------------------- |
| `hidden`             | Not rendered (any value, including `until-found`)                             |
| `aria-hidden="true"` | Hidden from the accessibility tree: decorative icons, sizing ghosts, overlays |

Content that a page hides from assistive technology is decorative, collapsed, or a layout duplicate of visible text, so it is stripped the
same way. Elements with `aria-hidden="false"` are kept.
...
const { markdown } = convert(html, {
    extract: {
        ...
        keepHidden: true // Keep hidden and aria-hidden elements
    }
});
```

This is useful when the page header or navigation contains important content, or when a page hides text it still wants read.
